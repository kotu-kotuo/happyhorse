...
const ReviewListItem = (props) => {
  ...
  return (
    <div className="flex my-6">
      ...
      />
      <div>
        {review.postUserID === review.reviewerID && review.rating === "good" ? (
        ...
        <div className="text-gray-900 text-base">{review.reviewerName}</div>
        <div className="text-gray-800 text-sm mb-1.5">{review.reviewText}</div>
        <div className="text-gray-500 reviewPostTitleText -mb-0.5">
          {createdTime(review.createdAt)}
        </div>
        <Link href={`/post/postShow/${review.postID}`}>

        <div className="text-blue-700 reviewPostTitleText border-b border-blue-700 mr-auto inline-block -mt-10 cursor-pointer">
          {review.postTitle}
        </div>
        </Link>
      </div>
